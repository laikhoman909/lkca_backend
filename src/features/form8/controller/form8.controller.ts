...
export class Form8Controller {
  ...
  @Put(':id/bank')
  async updateBankById(@Param('id', ParseIntPipe) id: number, @Body() dto: CreateForm8Dto) {
    const result = await this.formService.updateBankById(id, dto, dto.bankId);
    return { success: true, message: 'Bank data updated successfully', data: result };
  }
